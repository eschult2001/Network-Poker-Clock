...
public class FadeDemo2
{
	...
	ImagePanel(String file) {
	    ...
	    this.image = createImageIcon(file,"").getImage();
	...
	/**
	 * This method will be called from a worker thread with the value 
	 * computed by initFrameRate.  
	 */
	private synchronized void setFrameRate(long frameRate) {
	    this.frameRate = frameRate;
	}


	/** 
	 * Returns the frame rate computed by <code>initFrameRate</code>.
	 * @see #setFrameRate
	 */
	public synchronized long getFrameRate() {
	    return frameRate;
	}
	...
	final Action fadeAction = new AbstractAction("Fade") {
	    /**
		 * 
		 */
		private static final long serialVersionUID = 1L;
	...
	fadeAction.setEnabled(false);

	JToolBar toolbar = new JToolBar();
	toolbar.add(fadeAction);
	toolbar.addSeparator();
	toolbar.add(new JLabel("Total Fade Time (ms): "));
	toolbar.add(totalTimeSlider);

	final JLabel rateLabel = new JLabel("Computing frame rate...");
	...
	contentPane.add(rateLabel, BorderLayout.SOUTH);
	f.pack();
	f.setVisible(true);

	/* Before enabling the fade action, we have to initialize
	 * the frame rate.  This is done in a (Swing) worker thread.
	 */
	SwingWorker doComputeRate = new SwingWorker() {
	    public Object construct() {
		imagePanel.initFrameRate();
		return null;
	    }
	    public void finished() {
		fadeAction.setEnabled(true);
		rateLabel.setText("Frame Rate: " + imagePanel.getFrameRate());
	    }
	};
	doComputeRate.start();
    }
